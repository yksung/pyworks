...
slackUserName = "posneg_classifier-hankyung"
...
async def update_emotions(index_name, emotion_id, pos_neg):
    ...
    request_body = {
      "script":{
        "inline": "ctx._source.put('emotion_type', params.emotion_type); ctx._source.put('upd_datetime', params.upd_datetime);",
        "lang": "painless",
    ...
    r = await es.update(index=index_name, doc_type=TYPE_DOC, id=emotion_id, body=request_body, refresh=True)
...
def main(tousflux_ip, tousflux_port, number_of_modules, module_number):
      ...
      "query": {
        "bool": {
          ...
          "filter": {
            "script": {
              "script": {
                "inline": "def docDateTime=doc['doc_datetime'].getValue(); docDateTime/100000 % params.number_of_modules == params.module_number",
                "lang": "painless",
                "params": {
    ...
    for page in range(math.ceil(total/PAGE_SIZE)):
        ...
        try:
            ...
            if 'hits' in es_result and es_result['hits']['total']>0:
                ...
                for idx, doc in enumerate(resultlist):
                    ...
                    loop.run_until_complete(asyncio.ensure_future( update_emotions(doc['_index'], doc['_id'], request2tousflux(tousflux_ip, tousflux_port, str(idx+1).zfill(2), doc['_source']['matched_text']['string']) ) ))
