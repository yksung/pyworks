...
async def make_bulk(class_term_tuple, data):
    ...
    if len(term.strip())>0:
        ...
        bulk_body = {
            "_op_type": "update",
            "_index": await find_to_which_index(topic_id),
            "_type": TYPE_DOC,
            "_id": topic_id,
            "_source": {
                "script": {
...
async def find_to_which_index(topic_id):
    ...
    es = Elasticsearch(['%s:%d'%(es_ip, es_port)])
    es_result = await es.search(TOPICS_TO_SEARCH, doc_type="doc", body=request_body)
    '''
    es_conn = http.client.HTTPConnection(es_ip, es_port)
...
def main(project_seqs, start_date, end_date):
    ...
    try:
        es_conn = http.client.HTTPConnection(es_ip, es_port)
        es_conn.request("POST", "/"+INDEX_DOCUMENTS+"/"+TYPE_DOC+"/_search", json.dumps(es_request), { "Content-Type" : "application/json" })
        ...
        if 'hits' not in json.loads(es_result):
            logger.error("[main] 'hits' not in the result.")
            return
        elif total <= 0:
