...
'''
def get_data_for_report_trend(type_cd, trend_grp_seq, start_date, start_time, end_date, end_time):
    conn = pymysql.connect(host=mariadb_ip, port=mariadb_port, user=mariadb_user, password=mariadb_password, db=mariadb_db, charset=mariadb_charset, connect_timeout=60)
    curs = conn.cursor()
    ...
    sql = '''
         select d.search_day as search_day
           ...
           from tb_trend_group a,
                ...
                tb_naver_keyword d
          where a.seq = %d
            and d.type_cd = '%s' /*SCT001:자동완성어, SCT002:연관검색어*/
            and concat(d.search_day, d.search_time, '0000') >= concat('%s', '%s')
            and concat(d.search_day, d.search_time, '0000') <= concat('%s', '%s')
          ...
          order by d.search_day ASC, d.search_time ASC, a.seq ASC, b.seq asc, c.seq asc ''' % (
            trend_grp_seq, type_cd, start_date, start_time, end_date, end_time
           )
...
def get_every_dataset_for(project_seq):
    ...
    sql = """select seq from tb_dataset where project_seq=%s and use_yn='Y'"""
...
def get_include_keywords(dataset_seq):
    ...
    sql = """select keyword 
                  ...
                  , standard_keyword
                  , standard_title_filter_keywords
                  , standard_content_filter_keywords
                  , filter_urls
