...
BASE_EXCEL_DIRECTORY=r'E:\documents'
...
if __name__ == '__main__':
    ...
    for req in dao.get_excel_request():
        ...
        start_date = re.compile("[-:\s]").sub("", req['start_date'])[:8]
        end_date = re.compile("[-:\s]").sub("", req['end_date'])[:8]
        
        #02. 해당 엑셀에 대해서는 table의 상태값을 바꿈.
        dao.update_queue(req, "P")
        
        #03. 각 요청에 해당하는 디렉토리를 찾고(요청시퀀스) 없으면 생성. 파일이름 만듦.
        # 디렉토리 구성 ==> /data/dmap-data/dmap-excel/20170601/12'
        file_path = search_create_directory( os.path.join( BASE_EXCEL_DIRECTORY, reg_dt, str(req_seq) ) )
        file_name = "_".join([str(req_seq), csv_type_name, channel, start_date, end_date])
        
        #04. 데이터 검색 및 파일 작성
        if 'datasets' in req:
            for dataset_seq in req['datasets'].split(","): # dataset 하나당 엑셀 하나를 만듦. 따라서 dataset_seq는 한개씩만 넘어감.
                this_file_name = file_name + "_"+str(dataset_seq)
                ...
                if "hits" in result and result["hits"]["total"] > 0:
                    workbook = xlsxwriter.Workbook(os.path.join(file_path, this_file_name+".xlsx"), options={'strings_to_urls': False} )
                    ...
                    for row, result in enumerate(result["hits"]["hits"]):
                        for col, field in enumerate(crawl_doc_fields):
                            worksheet.write(row+1, col, result["_source"][field])
                    ...
                    if "hits" in result and result["hits"]["total"] > size:
                        for page in range(1, math.ceil(result["hits"]["total"]/size)): # 0, 1, 2, ....
                            ...
                            workbook = xlsxwriter.Workbook(os.path.join(file_path, this_file_name+"("+str(page)+")"+".xlsx"), options={'strings_to_urls': False} )
                            worksheet = workbook.add_worksheet()
